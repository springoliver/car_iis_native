...
internal class DynamicILGenerator : ILGenerator
{
	...
	private bool ProfileAPICheck => ((DynamicMethod)m_methodBuilder).ProfileAPICheck;
	...
	public override LocalBuilder DeclareLocal(Type localType, bool pinned)
	{
		...
		if (ProfileAPICheck && (runtimeType.InvocationFlags & INVOCATION_FLAGS.INVOCATION_FLAGS_NON_W8P_FX_API) != INVOCATION_FLAGS.INVOCATION_FLAGS_UNKNOWN)
		{
	...
	private int GetMemberRefToken(MethodBase methodInfo, Type[] optionalParameterTypes)
	{
		...
		SignatureHelper memberRefSignature = GetMemberRefSignature(methodInfo.CallingConvention, MethodBuilder.GetMethodBaseReturnType(methodInfo), array, optionalParameterTypes);
		if (runtimeMethodInfo != null)
		{
	...
	{
		if (ProfileAPICheck && (rtType.InvocationFlags & INVOCATION_FLAGS.INVOCATION_FLAGS_NON_W8P_FX_API) != INVOCATION_FLAGS.INVOCATION_FLAGS_UNKNOWN)
		{
	...
	{
		if (ProfileAPICheck && (rtMeth.InvocationFlags & INVOCATION_FLAGS.INVOCATION_FLAGS_NON_W8P_FX_API) != INVOCATION_FLAGS.INVOCATION_FLAGS_UNKNOWN)
		{
