		...
		else
		{
			...
			for (int k = 0; k < serializableMembers.Length; k++)
			{
				...
				if (!hashtable.Contains(name))
				{
					...
					{
						throw new SerializationException(Environment.GetResourceString("Serialization_MissingMember", serializableMembers[k], serObj.GetType(), typeof(OptionalFieldAttribute).FullName));
					}
				}
				else
				{
