...
internal static class StubHelpers
{
	...
	internal static IntPtr AddToCleanupList(ref CleanupWorkList pCleanupWorkList, SafeHandle handle)
	{
		...
		return SafeHandleAddRef(handle, ref cleanupWorkListElement.m_owned);
	}
	...
	{
		Exception ex = InternalGetCOMHRExceptionObject(hr, pCPCMD, pThis, fForWinRT: false);
		ex.InternalPreserveStackTrace();
		return ex;
	}
	...
	{
		Exception ex = InternalGetCOMHRExceptionObject(hr, pCPCMD, pThis, fForWinRT: true);
		ex.InternalPreserveStackTrace();
		return ex;
	}
