...
public abstract class DirectoryObjectSecurity : ObjectSecurity
{
	...
	private AuthorizationRuleCollection GetRules(bool access, bool includeExplicit, bool includeInherited, Type targetType)
	{
		...
		try
		{
			...
			if (access)
			{
				if ((_securityDescriptor.ControlFlags & ControlFlags.DiscretionaryAclPresent) != ControlFlags.None)
				{
					commonAcl = _securityDescriptor.DiscretionaryAcl;
				}
			}
			else if ((_securityDescriptor.ControlFlags & ControlFlags.SystemAclPresent) != ControlFlags.None)
			{
			...
			for (int j = 0; j < commonAcl.Count; j++)
			{
				...
				if ((!includeExplicit || (qualifiedAce2.AceFlags & AceFlags.Inherited) != AceFlags.None) && (!includeInherited || (qualifiedAce2.AceFlags & AceFlags.Inherited) == 0))
				{
					continue;
				}
				...
				if (access)
				{
					AccessControlType type = ((qualifiedAce2.AceQualifier != AceQualifier.AccessAllowed) ? AccessControlType.Deny : AccessControlType.Allow);
	...
	private bool ModifyAccess(AccessControlModification modification, ObjectAccessRule rule, out bool modified)
	{
		...
		else if ((modification == AccessControlModification.Add || modification == AccessControlModification.Set || modification == AccessControlModification.Reset) && rule.ObjectFlags != ObjectAceFlags.None && _securityDescriptor.DiscretionaryAcl.Revision < GenericAcl.AclRevisionDS)
		{
	...
	private bool ModifyAudit(AccessControlModification modification, ObjectAuditRule rule, out bool modified)
	{
		...
		else if ((modification == AccessControlModification.Add || modification == AccessControlModification.Set || modification == AccessControlModification.Reset) && rule.ObjectFlags != ObjectAceFlags.None && _securityDescriptor.SystemAcl.Revision < GenericAcl.AclRevisionDS)
		{
