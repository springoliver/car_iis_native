...
public abstract class CommonObjectSecurity : ObjectSecurity
{
		...
		try
		{
			...
			for (int j = 0; j < commonAcl.Count; j++)
			{
				...
				if (AceNeedsTranslation(commonAce2, access, includeExplicit, includeInherited))
				{
					...
					if (access)
					{
						authorizationRuleCollection.AddRule(AccessRuleFactory(type: (commonAce2.AceQualifier != AceQualifier.AccessAllowed) ? AccessControlType.Deny : AccessControlType.Allow, identityReference: identityReference, accessMask: commonAce2.AccessMask, isInherited: commonAce2.IsInherited, inheritanceFlags: commonAce2.InheritanceFlags, propagationFlags: commonAce2.PropagationFlags));
					}
					else
					{
	...
	private bool AceNeedsTranslation(CommonAce ace, bool isAccessAce, bool includeExplicit, bool includeInherited)
	{
		...
		if ((includeExplicit && (ace.AceFlags & AceFlags.Inherited) == 0) || (includeInherited && (ace.AceFlags & AceFlags.Inherited) != AceFlags.None))
		{
			return true;
		}
		return false;
