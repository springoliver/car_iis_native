...
{
	internal struct DateMapping(int MonthsLengthFlags, int GYear, int GMonth, int GDay)
	{
		internal int HijriMonthsLengthFlags = MonthsLengthFlags;

		internal DateTime GregorianDate = new DateTime(GYear, GMonth, GDay);
	}
	...
	private static void ConvertHijriToGregorian(int HijriYear, int HijriMonth, int HijriDay, ref int yg, ref int mg, ref int dg)
	{
		...
		gregorianDate.AddDays(num).GetDatePart(out yg, out mg, out dg);
	}
