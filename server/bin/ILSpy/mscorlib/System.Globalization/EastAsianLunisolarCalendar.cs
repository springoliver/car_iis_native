...
public abstract class EastAsianLunisolarCalendar : Calendar
{
	...
	public override int TwoDigitYearMax
	{
		...
		set
		{
			...
			{
				throw new ArgumentOutOfRangeException("value", Environment.GetResourceString("ArgumentOutOfRange_Range", 99, MaxCalendarYear));
			}
	...
	internal int CheckYearRange(int year, int era)
	{
		...
		{
			throw new ArgumentOutOfRangeException("year", Environment.GetResourceString("ArgumentOutOfRange_Range", MinEraCalendarYear(era), MaxEraCalendarYear(era)));
		}
		return year;
	}
	...
	public override DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era)
	{
		...
		if (day < 1 || day > num)
		{
			throw new ArgumentOutOfRangeException("day", Environment.GetResourceString("ArgumentOutOfRange_Day", num, month));
		}
		int nSolarYear = 0;
		int nSolarMonth = 0;
		int nSolarDay = 0;
	...
	{
		int num = GregorianIsLeapYear(nSYear);
		int num2 = ((num == 1) ? DaysToMonth366[nSMonth - 1] : DaysToMonth365[nSMonth - 1]);
		num2 += nSDate;
		int num3 = num2;
		nLYear = nSYear;
		int yearInfo;
		...
		{
			nLYear--;
			num3 += ((GregorianIsLeapYear(nLYear) == 1) ? 366 : 365);
		...
		num3 -= DaysToMonth365[yearInfo - 1];
		num3 -= yearInfo2 - 1;
		int num4 = 32768;
		int yearInfo3 = GetYearInfo(nLYear, 3);
		int num5 = (((yearInfo3 & num4) != 0) ? 30 : 29);
		nLMonth = 1;
		while (num3 > num5)
		{
			num3 -= num5;
