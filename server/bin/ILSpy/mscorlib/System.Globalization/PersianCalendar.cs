...
public class PersianCalendar : Calendar
{
	...
	private long GetAbsoluteDatePersian(int year, int month, int day)
	{
		if (year >= 1 && year <= 9378 && month >= 1 && month <= 12)
		{
			...
			long num3 = CalendricalCalculationsHelper.PersianNewYearOnOrBefore(PersianEpoch + num2 + 180);
			return num3 + num;
		}
	...
	internal int GetDatePart(long ticks, int part)
	{
		...
		int num3 = (int)Math.Floor((double)(num2 - PersianEpoch) / 365.242189 + 0.5) + 1;
		if (part == 0)
		{
			return num3;
		}
