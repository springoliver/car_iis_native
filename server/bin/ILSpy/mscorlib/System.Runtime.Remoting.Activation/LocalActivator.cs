...
internal class LocalActivator : ContextAttribute, IActivator
{
	...
	public override bool IsContextOK(Context ctx, IConstructionCallMessage ctorMsg)
	{
		...
		string text = RemotingConfigHandler.IsWellKnownClientType(runtimeType)?.ObjectUrl;
		if (text != null)
		{
		...
		if (activatedClientTypeEntry == null)
		{
			...
			if (callSiteActivationAttributes != null)
			{
				...
				{
					if (callSiteActivationAttributes[i] is UrlAttribute urlAttribute)
					{
		...
		string text3 = null;
		text3 = (text2.EndsWith("/", StringComparison.Ordinal) ? (text2 + "RemoteActivationService.rem") : (text2 + "/RemoteActivationService.rem"));
		...
		if (ctorMsg.Properties.Contains("Permission"))
		{
			...
			if (activationType.IsContextful)
			{
				...
				if (contextProperties != null && contextProperties.Count > 0)
				{
					...
					activationAttributes = new object[1] { remotePropertyHolderAttribute };
				}
			}
			MethodBase methodBase = GetMethodBase(ctorMsg);
