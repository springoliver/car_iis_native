...
internal static class LongPathFile
{
	...
	internal static void Copy(string sourceFileName, string destFileName, bool overwrite)
	{
		...
		FileIOPermission.QuickDemand(FileIOPermissionAccess.Read, text, checkForDuplicates: false, needFullPath: false);
		string text2 = LongPath.NormalizePath(destFileName);
		FileIOPermission.QuickDemand(FileIOPermissionAccess.Write, text2, checkForDuplicates: false, needFullPath: false);
		InternalCopy(text, text2, sourceFileName, destFileName, overwrite);
	}
	...
	private static string InternalCopy(string fullSourceFileName, string fullDestFileName, string sourceFileName, string destFileName, bool overwrite)
	{
		...
		if (!Win32Native.CopyFile(fullSourceFileName, fullDestFileName, !overwrite))
		{
			...
			if (lastWin32Error != 80)
			{
				...
				{
					throw new IOException(Environment.GetResourceString("Arg_FileIsDirectory_Name", destFileName), 5, fullDestFileName);
				}
			}
	...
	internal static void Delete(string path)
	{
		...
		FileIOPermission.QuickDemand(FileIOPermissionAccess.Write, text, checkForDuplicates: false, needFullPath: false);
	...
	internal static DateTimeOffset GetCreationTime(string path)
	{
		...
		int num = File.FillAttributeInfo(path2, ref data, tryagain: false, returnErrorOnNotFound: false);
		if (num != 0)
		{
			__Error.WinIOError(num, text);
		}
		DateTime dateTime = DateTime.FromFileTimeUtc(data.ftCreationTime.ToTicks()).ToLocalTime();
	...
	internal static DateTimeOffset GetLastAccessTime(string path)
	{
		...
		DateTime dateTime = DateTime.FromFileTimeUtc(data.ftLastAccessTime.ToTicks()).ToLocalTime();
		return new DateTimeOffset(dateTime).ToLocalTime();
	}
	...
	internal static DateTimeOffset GetLastWriteTime(string path)
	{
		...
		DateTime dateTime = DateTime.FromFileTimeUtc(data.ftLastWriteTime.ToTicks()).ToLocalTime();
		return new DateTimeOffset(dateTime).ToLocalTime();
	}
	...
	{
		string text = LongPath.NormalizePath(sourceFileName);
		FileIOPermission.QuickDemand(FileIOPermissionAccess.Read | FileIOPermissionAccess.Write, text, checkForDuplicates: false, needFullPath: false);
		string text2 = LongPath.NormalizePath(destFileName);
		FileIOPermission.QuickDemand(FileIOPermissionAccess.Write, text2, checkForDuplicates: false, needFullPath: false);
	...
	internal static long GetLength(string path)
	{
		...
		string path2 = Path.AddLongPathPrefix(text);
		Win32Native.WIN32_FILE_ATTRIBUTE_DATA data = default(Win32Native.WIN32_FILE_ATTRIBUTE_DATA);
		int num = File.FillAttributeInfo(path2, ref data, tryagain: false, returnErrorOnNotFound: true);
		if (num != 0)
		{
		...
		return ((long)data.fileSizeHigh << 32) | (data.fileSizeLow & 0xFFFFFFFFu);
	}
}
