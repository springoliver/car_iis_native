...
internal class ISCIIEncoding : EncodingNLS, ISerializable
{
	...
	private static int[] UnicodeToIndicChar = new int[1135]
	{
	...
	private static byte[] SecondIndicByte = new byte[4] { 0, 233, 184, 191 };

	private static int[] IndicMappingIndex = new int[12]
	{
		-1, -1, 0, 1, 2, 3, 1, 4, 5, 6,
		7, 8
	};

	private static char[,,] IndicMapping = new char[9, 2, 96]
	{
		{
			{
				'\0', '\u0901', '\u0902', '\u0903', 'अ', 'आ', 'इ', 'ई', 'उ', 'ऊ',
	...
	internal unsafe override int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, EncoderNLS baseEncoder)
	{
		...
		while (encodingByteBuffer.MoreData)
		{
			...
			if (num3 != num)
			{
				if (!encodingByteBuffer.AddByte(239, (byte)(num3 | 0x40)))
				{
					break;
				}
				num = num3;
		...
		if (num != defaultCodePage && (iSCIIEncoder == null || iSCIIEncoder.MustFlush))
		{
			if (encodingByteBuffer.AddByte(239, (byte)(defaultCodePage | 0x40)))
			{
				num = defaultCodePage;
			}
			else
	...
	internal unsafe override int GetChars(byte* bytes, int byteCount, char* chars, int charCount, DecoderNLS baseDecoder)
	{
		...
		while (encodingCharBuffer.MoreData)
		{
			...
			char c3 = IndicMapping[num2, 0, nextByte - 160];
			char c4 = IndicMapping[num2, 1, nextByte - 160];
		...
		if (iSCIIDecoder != null && chars != null)
		{
			...
			{
				iSCIIDecoder.currentCodePage = defaultCodePage;
	...
	{
		return defaultCodePage + base.EncoderFallback.GetHashCode() + base.DecoderFallback.GetHashCode();
	}
}
