...
internal sealed class GB18030Encoding : DBCSCodePageEncoding, ISerializable
{
	...
	internal sealed class GB18030Decoder : DecoderNLS, ISerializable
	{
		...
		internal GB18030Decoder(SerializationInfo info, StreamingContext context)
		{
			...
			try
			{
				...
				bLeftOver1 = (short)info.GetValue("bLeftOver1", typeof(short));
				bLeftOver2 = (short)info.GetValue("bLeftOver2", typeof(short));
				bLeftOver3 = (short)info.GetValue("bLeftOver3", typeof(short));
				bLeftOver4 = (short)info.GetValue("bLeftOver4", typeof(short));
			}
			catch (SerializationException)
			{
				m_encoding = new GB18030Encoding();
			}
		}
		...
		void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue("bLeftOver1", bLeftOver1);
			info.AddValue("bLeftOver2", bLeftOver2);
			info.AddValue("bLeftOver3", bLeftOver3);
			info.AddValue("bLeftOver4", bLeftOver4);
	...
	{
		bFlagDataTable = false;
		iExtraBytes = 87032;
		base.LoadManagedCodePage();
		byte* ptr = (byte*)(void*)safeMemorySectionHandle.DangerousGetHandle();
		mapUnicodeTo4BytesFlags = ptr + 262144;
		map4BytesToUnicode = (char*)(ptr + 262144) + 4096;
		if (*mapCodePageCached == CodePage)
		{
			return;
		}
		char c = '\0';
		ushort num = 0;
		for (int i = 0; i < tableUnicodeToGBDiffs.Length; i++)
		{
			ushort num2 = tableUnicodeToGBDiffs[i];
			if ((num2 & 0x8000) != 0)
			{
			...
			if (encodingByteBuffer.MoreData)
			{
				...
				else
				{
					...
					else
					{
						...
						else if (encodingByteBuffer.AddByte((byte)(num2 >> 8), (byte)(num2 & 0xFF)))
						{
							continue;
						}
					}
