...
internal class ISO2022Encoding : DBCSCodePageEncoding
{
	...
	internal ISO2022Encoding(int codePage)
		: base(codePage, tableBaseCodePages[codePage % 10])
	{
		m_bUseMlangTypeForSerialization = true;
	}
	...
	protected unsafe override string GetMemorySectionName()
	{
		...
		return string.Format(CultureInfo.InvariantCulture, format, num, pCodePage->VersionMajor, pCodePage->VersionMinor, pCodePage->VersionRevision, pCodePage->VersionBuild);
	}
	...
	private unsafe int GetBytesCP5022xJP(char* chars, int charCount, byte* bytes, int byteCount, ISO2022Encoder encoder)
	{
		...
		while (encodingByteBuffer.MoreData)
		{
			...
			while (true)
			{
				...
				if (CodePage == 50220)
				{
					if (b2 >= 33 && b2 < 33 + HalfToFullWidthKanaTable.Length)
					{
						num = (ushort)(HalfToFullWidthKanaTable[b2 - 33] & 0x7F7F);
						continue;
					}
					goto IL_009a;
				}
			...
			continue;
			IL_00be:
			if (iSO2022Modes != ISO2022Modes.ModeHalfwidthKatakana)
			{
				if (CodePage == 50222)
				{
			...
			if (!encodingByteBuffer.AddByte((byte)(b2 & 0x7F)))
			{
				break;
			}
		}
		if (iSO2022Modes != ISO2022Modes.ModeASCII && (encoder == null || encoder.MustFlush))
		{
			...
			if (iSO2022Modes != ISO2022Modes.ModeASCII && (CodePage != 50222 || iSO2022Modes != ISO2022Modes.ModeHalfwidthKatakana))
			{
				if (encodingByteBuffer.AddByte((byte)27, (byte)40, (byte)66))
				{
					iSO2022Modes = ISO2022Modes.ModeASCII;
	...
	private unsafe int GetBytesCP52936(char* chars, int charCount, byte* bytes, int byteCount, ISO2022Encoder encoder)
	{
		...
		while (encodingByteBuffer.MoreData)
		{
			...
			if (b != 0)
			{
				...
				if (encodingByteBuffer.AddByte((byte)(b & 0x7F), (byte)(b2 & 0x7F)))
				{
					continue;
				}
				break;
