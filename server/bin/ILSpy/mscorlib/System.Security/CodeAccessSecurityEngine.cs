...
internal static class CodeAccessSecurityEngine
{
	...
	{
		asm.GetGrantSet(out var newGrant, out var newDenied);
		CheckHelper(newGrant, newDenied, demand, PermissionToken.GetToken(demand), RuntimeMethodHandleInternal.EmptyHandle, asm, SecurityAction.Demand, throwException: true);
	}
	...
	{
		FrameSecurityDescriptor frameSecurityDescriptor = CheckNReturnSO(AssertPermissionToken, AssertPermission, ref stackMark, 1);
	...
	internal static bool TryResolveGrantSet(Evidence evidence, out PermissionSet grantSet)
	{
		...
		if ((hostSecurityManager.Flags & HostSecurityManagerOptions.HostResolvePolicy) == HostSecurityManagerOptions.HostResolvePolicy)
		{
			...
			{
				throw new PolicyException(Environment.GetResourceString("Policy_NullHostGrantSet", hostSecurityManager.GetType().FullName));
			}
			if (AppDomain.CurrentDomain.IsHomogenous)
			{
				...
				{
					throw new PolicyException(Environment.GetResourceString("Policy_GrantSetDoesNotMatchDomain", hostSecurityManager.GetType().FullName));
				}
			}
