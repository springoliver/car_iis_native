...
internal class ActivityTracker
{
	private class ActivityInfo
	{
		...
		{
			if ((m_eventOptions & EventActivityOptions.Detachable) != EventActivityOptions.None)
			{
				return true;
			}
			return false;
		...
		private unsafe static int AddIdToGuid(Guid* outPtr, int whereToAddId, uint id, bool overflow = false)
		{
			...
			((int*)outPtr)[3] = (int)(*(uint*)outPtr + ((uint*)outPtr)[1] + ((uint*)outPtr)[2] + 1503500717);
	...
	public void OnStart(string providerName, string activityName, int task, ref Guid activityId, ref Guid relatedActivityId, EventActivityOptions options)
	{
		...
		if (log.Debug)
		{
			log.DebugFacilityMessage("OnStartRetActivityState", ActivityInfo.LiveActivities(activityInfo2));
