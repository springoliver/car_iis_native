...
internal static class AsyncLocalValueMap
{
	...
	private sealed class ManyElementAsyncLocalValueMap : Dictionary<IAsyncLocal, object>, IAsyncLocalValueMap
	{
		...
		public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
		{
			...
			if (value != null || !treatNullValueAsNonexistent)
			{
				ManyElementAsyncLocalValueMap manyElementAsyncLocalValueMap = new ManyElementAsyncLocalValueMap(num + ((!flag) ? 1 : 0));
			...
			if (flag)
			{
				if (num == 17)
				{
					...
					while (enumerator2.MoveNext())
					{
						...
						if (key != current2.Key)
						{
							multiElementAsyncLocalValueMap.UnsafeStore(num2++, current2.Key, current2.Value);
						}
					}
					return multiElementAsyncLocalValueMap;
				}
				ManyElementAsyncLocalValueMap manyElementAsyncLocalValueMap2 = new ManyElementAsyncLocalValueMap(num - 1);
