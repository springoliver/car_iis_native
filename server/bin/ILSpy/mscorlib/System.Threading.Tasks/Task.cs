...
public class Task<TResult> : Task
{
	...
	private string DebuggerDisplayMethodDescription
	{
		get
		{
			Delegate obj = (Delegate)m_action;
			if ((object)obj == null)
			{
				return "{null}";
			}
...
public class Task : IThreadPoolWorkItem, IAsyncResult, IDisposable
{
	...
	public bool IsCanceled
	{
		...
		get
		{
			return (m_stateFlags & 0x600000) == 4194304;
		}
	}
	...
	public TaskCreationOptions CreationOptions
	{
		...
		get
		{
			return Options & (TaskCreationOptions)(-65281);
		}
	}
	...
	internal void TaskConstructorCore(object action, object state, CancellationToken cancellationToken, TaskCreationOptions creationOptions, InternalTaskOptions internalOptions, TaskScheduler scheduler)
	{
		...
		if (m_action == null || (internalOptions & InternalTaskOptions.ContinuationTask) != InternalTaskOptions.None)
		{
			num |= 0x2000000;
		}
		m_stateFlags = num;
	...
	internal bool FireTaskScheduledIfNeeded(TaskScheduler ts)
	{
		...
		if (log.IsEnabled() && (m_stateFlags & 0x40000000) == 0)
		{
			m_stateFlags |= 1073741824;
			Task internalCurrent = InternalCurrent;
			Task parent = m_parent;
			log.TaskScheduled(ts.Id, internalCurrent?.Id ?? 0, Id, parent?.Id ?? 0, (int)Options, Thread.GetDomainID());
			return true;
		}
		return false;
	}
	...
	{
		if (bPreventDoubleExecution || (Options & (TaskCreationOptions)2048) != TaskCreationOptions.None)
		{
			int oldFlags = 0;
	...
	private void ExecuteWithThreadLocal(ref Task currentTaskSlot)
	{
		...
		if (flag)
		{
			...
			{
				EventSource.SetCurrentThreadActivityId(TplEtwProvider.CreateGuidForTaskID(Id), out oldActivityThatWillContinue);
			}
			if (task != null)
			{
	...
	internal bool InternalWait(int millisecondsTimeout, CancellationToken cancellationToken)
	{
		...
		{
			Task internalCurrent = InternalCurrent;
			log.TaskWaitBegin(internalCurrent?.m_taskScheduler.Id ?? TaskScheduler.Default.Id, internalCurrent?.Id ?? 0, Id, TplEtwProvider.TaskWaitBehavior.Synchronous, 0, Thread.GetDomainID());
		}
		bool flag2 = IsCompleted;
		if (!flag2)
		{
			Debugger.NotifyOfCrossThreadDependency();
			flag2 = (millisecondsTimeout == -1 && !cancellationToken.CanBeCanceled && WrappedTryRunInline() && IsCompleted) || SpinThenBlockingWait(millisecondsTimeout, cancellationToken);
		}
		if (flag)
		{
	...
	internal void FinishContinuations()
	{
		...
		TplEtwProvider.Log.RunningContinuation(Id, obj);
		if (obj == null)
		{
			return;
		}
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				TplEtwProvider.Log.RunningContinuationList(Id, i, standardTaskContinuation);
				list[i] = null;
		...
		for (int j = 0; j < count; j++)
		{
			...
			list[j] = null;
			TplEtwProvider.Log.RunningContinuationList(Id, j, obj2);
	...
	internal void ContinueWithCore(Task continuationTask, TaskScheduler scheduler, CancellationToken cancellationToken, TaskContinuationOptions options)
	{
		...
		if ((Options & (TaskCreationOptions)1024) != TaskCreationOptions.None && !(this is ITaskCompletionAction))
		{
			TplEtwProvider log = TplEtwProvider.Log;
			if (log.IsEnabled())
			{
	...
	public static Task<Task<TResult>> WhenAny<TResult>(params Task<TResult>[] tasks)
	{
		...
		return task.ContinueWith(Task<TResult>.TaskWhenAnyCast, default(CancellationToken), TaskContinuationOptions.DenyChildAttach | TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
	}
