...
internal class GenericArraySortHelper<T> : IArraySortHelper<T> where T : IComparable<T>
{
	public void Sort(T[] keys, int index, int length, IComparer<T> comparer)
	{
		try
		{
			if (comparer == null || comparer == Comparer<T>.Default)
			{
				...
				{
					DepthLimitedQuickSort(keys, index, length + index - 1, 32);
				}
			}
	...
	private static void DepthLimitedQuickSort(T[] keys, int left, int right, int depthLimit)
	{
		do
		{
			...
			int num2 = right;
			int num3 = num + (num2 - num >> 1);
			SwapIfGreaterWithItems(keys, num, num3);
			SwapIfGreaterWithItems(keys, num, num2);
			SwapIfGreaterWithItems(keys, num3, num2);
			T val = keys[num3];
			do
			{
				if (val == null)
			...
			if (num2 - left <= right - num)
			{
				if (left < num2)
				{
					DepthLimitedQuickSort(keys, left, num2, depthLimit);
				}
				left = num;
			}
			else
			{
				if (num < right)
				{
					DepthLimitedQuickSort(keys, num, right, depthLimit);
				}
				right = num2;
			}
		}
...
internal class GenericArraySortHelper<TKey, TValue> : IArraySortHelper<TKey, TValue> where TKey : IComparable<TKey>
{
	public void Sort(TKey[] keys, TValue[] values, int index, int length, IComparer<TKey> comparer)
	{
		try
		{
			if (comparer == null || comparer == Comparer<TKey>.Default)
			{
				...
				{
					DepthLimitedQuickSort(keys, values, index, length + index - 1, 32);
				}
			}
			else if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
			{
	...
	private static void DepthLimitedQuickSort(TKey[] keys, TValue[] values, int left, int right, int depthLimit)
	{
		do
		{
			...
			int num2 = right;
			int num3 = num + (num2 - num >> 1);
			SwapIfGreaterWithItems(keys, values, num, num3);
			SwapIfGreaterWithItems(keys, values, num, num2);
			SwapIfGreaterWithItems(keys, values, num3, num2);
			TKey val = keys[num3];
			do
			{
				if (val == null)
			...
			if (num2 - left <= right - num)
			{
				if (left < num2)
				{
					DepthLimitedQuickSort(keys, values, left, num2, depthLimit);
				}
				left = num;
			}
			else
			{
				if (num < right)
				{
					DepthLimitedQuickSort(keys, values, num, right, depthLimit);
				}
				right = num2;
			}
		}
