...
internal sealed class TypeNameParser : IDisposable
{
	...
	private static Assembly ResolveAssembly(string asmName, Func<AssemblyName, Assembly> assemblyResolver, bool throwOnError, ref StackCrawlMark stackMark)
	{
		...
		if (assemblyResolver == null)
		{
			if (throwOnError)
			{
				assembly = RuntimeAssembly.InternalLoad(asmName, null, ref stackMark, forIntrospection: false);
			}
			else
			{
				try
	...
	private static Type ResolveType(Assembly assembly, string[] names, Func<Assembly, string, bool, Type> typeResolver, bool throwOnError, bool ignoreCase, ref StackCrawlMark stackMark)
	{
		...
		{
			type = ((!(assembly == null)) ? assembly.GetType(text, throwOnError, ignoreCase) : RuntimeType.GetType(text, throwOnError, ignoreCase, reflectionOnly: false, ref stackMark));
		}
		else
		{
			...
			{
				string message = ((assembly == null) ? Environment.GetResourceString("TypeLoad_ResolveType", text) : Environment.GetResourceString("TypeLoad_ResolveTypeFromAssembly", text, assembly.FullName));
	...
	private static string EscapeTypeName(string name)
	{
		...
		{
			if (Array.IndexOf(SPECIAL_CHARS, value) >= 0)
			{
	...
	private static SafeTypeNameParserHandle CreateTypeNameParser(string typeName, bool throwOnError)
	{
		...
		_CreateTypeNameParser(typeName, JitHelpers.GetObjectHandleOnStack(ref o), throwOnError);
		return o;
	}
	...
	{
		string[] o = null;
		_GetNames(m_NativeParser, JitHelpers.GetObjectHandleOnStack(ref o));
		return o;
	}
	...
	{
		SafeTypeNameParserHandle[] o = null;
		_GetTypeArguments(m_NativeParser, JitHelpers.GetObjectHandleOnStack(ref o));
		return o;
	}
	...
	{
		int[] o = null;
		_GetModifiers(m_NativeParser, JitHelpers.GetObjectHandleOnStack(ref o));
		return o;
	}
	...
	{
		string s = null;
		_GetAssemblyName(m_NativeParser, JitHelpers.GetStringHandleOnStack(ref s));
		return s;
	}
}
