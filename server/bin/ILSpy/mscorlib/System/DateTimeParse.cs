...
internal static class DateTimeParse
{
		...
		case TokenType.YearNumberToken:
		{
			...
			case TokenType.SEP_Pm:
				if (raw.timeMark == TM.NotSet)
				{
					...
					if (dps == DS.D_NN && enableAmPmParseAdjustment && !ProcessTerminaltState(DS.DX_NN, ref result, ref styles, ref raw, dtfi))
					{
						return false;
					}
					raw.AddNumber(dtok.num);
	...
	private static bool GetDayOfYNN(ref DateTimeResult result, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
	{
		...
		if (GetYearMonthDayOrder(shortDatePattern, dtfi, out var order) && order == 3)
		{
	...
	internal static bool ProcessTerminaltState(DS dps, ref DateTimeResult result, ref DateTimeStyles styles, ref DateTimeRawInfo raw, DateTimeFormatInfo dtfi)
	{
		...
		case DS.DX_NNY:
			...
			break;
		case DS.DX_YMN:
			flag = GetDayOfYMN(ref result, ref raw, dtfi);
			break;
		case DS.DX_YN:
			flag = GetDayOfYN(ref result, ref raw, dtfi);
			break;
		case DS.DX_YM:
			flag = GetDayOfYM(ref result, ref raw, dtfi);
			break;
		case DS.TX_N:
			flag = GetTimeOfN(dtfi, ref result, ref raw);
			break;
		case DS.TX_NN:
			flag = GetTimeOfNN(dtfi, ref result, ref raw);
			break;
		case DS.TX_NNN:
			flag = GetTimeOfNNN(dtfi, ref result, ref raw);
			break;
		case DS.TX_TS:
			flag = true;
			break;
	...
	private static bool ParseByFormat(ref __DTString str, ref __DTString format, ref ParsingInfo parseInfo, DateTimeFormatInfo dtfi, ref DateTimeResult result)
	{
		...
		case 'f':
			...
			if (returnValue <= 7)
			{
				...
				{
					result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_RepeatDateTimePattern", c);
					return false;
				}
				break;
			}
