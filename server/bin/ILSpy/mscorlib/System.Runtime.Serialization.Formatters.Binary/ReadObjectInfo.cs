...
internal sealed class ReadObjectInfo
{
	...
	internal MemberInfo GetMemberInfo(string name)
	{
		...
		if (isSi)
		{
			throw new SerializationException(Environment.GetResourceString("Serialization_MemberInfo", string.Concat(objectType, " ", name)));
		}
		if (cache.memberInfos == null)
		{
			throw new SerializationException(Environment.GetResourceString("Serialization_NoMemberInfo", string.Concat(objectType, " ", name)));
		}
	...
	internal Type GetType(string name)
	{
		...
		if ((object)type == null)
		{
			throw new SerializationException(Environment.GetResourceString("Serialization_ISerializableTypes", string.Concat(objectType, " ", name)));
		}
		return type;
	}
	...
	internal Type[] GetMemberTypes(string[] inMemberNames, Type objectType)
	{
		...
		for (int j = 0; j < cache.memberInfos.Length; j++)
		{
			...
			if (!flag2)
			{
				...
				{
					throw new SerializationException(Environment.GetResourceString("Serialization_MissingMember", cache.memberNames[j], objectType, typeof(OptionalFieldAttribute).FullName));
				}
			}
		}
		return array;
	...
	private static ReadObjectInfo GetObjectInfo(SerObjectInfoInit serObjectInfoInit)
	{
		...
		readObjectInfo.objectInfoId = Interlocked.Increment(ref readObjectInfoCounter);
