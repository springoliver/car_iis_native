...
internal sealed class ObjectReader
{
	...
	internal ObjectReader(Stream stream, ISurrogateSelector selector, StreamingContext context, InternalFE formatterEnums, SerializationBinder binder)
	{
		...
		m_context = context;
		m_binder = binder;
		if (m_binder != null && m_binder is ResourceReader.TypeLimitingDeserializationBinder typeLimitingDeserializationBinder)
		{
	...
	{
		throw new SerializationException(Environment.GetResourceString("Serialization_ParseError", string.Concat(onStack.PRname, " ", onStack.PRparseTypeEnum, " ", processing.PRname, " ", processing.PRparseTypeEnum)));
	}
	...
	internal void CheckSecurity(ParseRecord pr)
	{
		...
		if ((object)pRdtType != null && IsRemoting)
		{
			...
			{
				throw new ArgumentException(Environment.GetResourceString("Serialization_MBRAsMBV", pRdtType.FullName));
			}
	...
	private void ParseArray(ParseRecord pr)
	{
		...
		else
		{
			...
			{
				throw new SerializationException(Environment.GetResourceString("Serialization_ArrayType", pr.PRarrayTypeEnum));
			}
			pr.PRisLowerBound = false;
