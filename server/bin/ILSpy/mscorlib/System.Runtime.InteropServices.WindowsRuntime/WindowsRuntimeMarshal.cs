...
public static class WindowsRuntimeMarshal
{
	...
	internal static class ManagedEventRegistrationImpl
	{
		...
		internal static void AddEventHandler<T>(Func<T, EventRegistrationToken> addMethod, Action<EventRegistrationToken> removeMethod, T handler)
		{
			...
			Dictionary<object, EventRegistrationTokenList> eventRegistrationTokenTable = GetEventRegistrationTokenTable(target, removeMethod);
			...
			try
			{
				...
				try
				{
					...
					lock (orCreateEventRegistrationTokenTable)
					{
						...
						object obj = orCreateEventRegistrationTokenTable.FindEquivalentKeyUnsafe(handler, out value);
						if (obj == null)
						{
	...
	{
		ManagedActivationFactory managedActivationFactory = GetManagedActivationFactory(type);
	...
	internal static IntPtr GetClassActivatorForApplication(string appBase)
	{
		if (s_pClassActivator == IntPtr.Zero)
		{
			...
			AppDomain appDomain = AppDomain.CreateDomain(Environment.GetResourceString("WinRTHostDomainName", appBase), null, info);
			...
			if (Interlocked.CompareExchange(ref s_pClassActivator, iWinRTClassActivator, IntPtr.Zero) != IntPtr.Zero)
			{
				Marshal.Release(iWinRTClassActivator);
				winRTClassActivator = null;
				try
				{
