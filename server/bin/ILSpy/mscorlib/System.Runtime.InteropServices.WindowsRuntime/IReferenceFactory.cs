...
internal static class IReferenceFactory
{
	...
	internal static object CreateIReference(object obj)
	{
		...
		{
			Type type2 = typeof(CLRIReferenceImpl<>).MakeGenericType(type);
			return Activator.CreateInstance(type2, propertyType.Value, obj);
		}
		return null;
	}
	...
	internal static object CreateIReferenceArray(Array obj)
	{
		...
		PropertyType? propertyType = null;
		if (elementType == s_pointType)
		{
			propertyType = PropertyType.PointArray;
		}
		else if (elementType == s_rectType)
		{
			propertyType = PropertyType.RectArray;
		}
		else if (elementType == s_sizeType)
		{
			propertyType = PropertyType.SizeArray;
		}
		else if (elementType.IsValueType)
		{
			...
			{
				propertyType = PropertyType.OtherArray;
			}
		}
		...
		if (propertyType.HasValue)
		{
			Type type = typeof(CLRIReferenceArrayImpl<>).MakeGenericType(elementType);
			return Activator.CreateInstance(type, propertyType.Value, obj);
		}
